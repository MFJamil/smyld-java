...
public class SMYLDToolTipManager extends MouseAdapter implements
		MouseMotionListener {
	 ...
	 * the mouse is, such as JTree. When the mouse moves into the JTree and over
	 * a region that has a valid tooltip, the tooltip will become visibile after
	 * <code>initialDelay</code> milliseconds. After <code>dismissDelay</code>
	 * milliseconds the tooltip will be hidden. If the mouse is over a region
	 * that has a valid tooltip, and the tooltip is currently visible, when the
	 * mouse moves to a region that doesn't have a valid tooltip the tooltip
	 * will be hidden. If the mouse then moves back into a region that has a
	 * valid tooltip within <code>reshowDelay</code> milliseconds, the tooltip
	 * will immediately be shown, otherwise the tooltip will be shown again
	 * after <code>initialDelay</code> milliseconds.
	 * 
	 ...
	 * @author Rich Schiavi
	 */
	final static SMYLDToolTipManager sharedInstance = new SMYLDToolTipManager();
			...
			public void actionPerformed(ActionEvent e) {
				if (tipWindow != null) { // showing we unshow
					...
					if (insideComponent != null) {
						...
						preferredLocation = new Point(10, insideComponent
								.getHeight() + 10); // manual set
						showTipWindow();
	...
	/**
	 * Returns true if lightweight (all-Java) <code>Tooltips</code> are in
	 * use, or false if heavyweight (native peer) <code>Tooltips</code> are
	 * being used.
	 * 
	 ...
	 * after <code>milliseconds</code> milliseconds, the user will have to
	 * wait an additional <code>initialDelay</code> milliseconds before the
	 * tooltip is shown again.
	 * 
	 * @param milliseconds
	...
	void showTipWindow() {
		...
		if (enabled) {
			...
			Rectangle sBounds = insideComponent.getGraphicsConfiguration()
					.getBounds();
			boolean leftToRight = insideComponent.getComponentOrientation()
					.isLeftToRight();
			...
			tip.setTipText("Hello this is me");
			/*
			 * if (tipComp!=null) tip.setComponent(tipComp); else
			 * tip.setTipText("Hello this is me");
			 */
			...
			if (!heavyWeightPopupEnabled) {
				...
				popupRect.setBounds(location.x, location.y, size.width,
						size.height);
			...
			if (location.x < sBounds.x) {
				...
				location.x = sBounds.x
						+ Math.max(0, sBounds.width - size.width);
			}
			if (location.y < sBounds.y) {
				location.y = sBounds.y;
			} else if (location.y - sBounds.y + size.height > sBounds.height) {
				location.y = sBounds.y
						+ Math.max(0, sBounds.height - size.height);
			}

			PopupFactory popupFactory = PopupFactory.getSharedInstance();

			tipWindow = popupFactory.getPopup(insideComponent, tipComp,
			...
			Window componentWindow = SwingUtilities
					.windowForComponent(insideComponent);
	 ...
	 * tool tip manager has a binding in any one of the <code>InputMaps</code>
	 * registered under the condition <code>WHEN_FOCUSED</code>.
	 * <p>
	 * This does not use <code>isFocusTraversable</code> as some components
	 * may override <code>isFocusTraversable</code> and base the return value
	 * on something other than bindings. For example, <code>JButton</code>
	...
	private boolean shouldRegisterBindings(JComponent component) {
		/*
		 * InputMap inputMap = component.getInputMap(JComponent.WHEN_FOCUSED, //
		 * false); while (inputMap != null && inputMap.size() == 0) { inputMap =
		 * inputMap.getParent(); } return (inputMap != null);
		 */
		return true;
	}
	...
	public void mouseExited(MouseEvent event) {
		...
		if (window != null && event.getSource() == window) {
			...
			Container insideComponentWindow = insideComponent
					.getTopLevelAncestor();
			Point location = event.getPoint();
			...
			if (win != null) { // insideComponent may have been hidden (e.g. in
				...
				Point location = SwingUtilities.convertPoint(insideComponent,
						event.getPoint(), win);
				Rectangle bounds = insideComponent.getTopLevelAncestor()
						.getBounds();
				location.x += bounds.x;
				location.y += bounds.y;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (insideComponent != null && insideComponent.isShowing()) {
				...
				if (toolTipText == null && mouseEvent != null) {
					...
					preferredLocation = insideComponent
	...
	private int getPopupFitWidth(Rectangle popupRectInScreen, Component invoker) {
		if (invoker != null) {
			Container parent;
			for (parent = invoker.getParent(); parent != null; parent = parent
					.getParent()) {
				...
				if (parent instanceof JFrame || parent instanceof JDialog
					...
					popupFrameRect
							.setBounds(p.x, p.y, parent.getBounds().width,
		...
		if (invoker != null) {
			...
			for (parent = invoker.getParent(); parent != null; parent = parent
				...
				if (parent instanceof JFrame || parent instanceof JDialog
					...
					return getHeightAdjust(parent.getBounds(),
							popupRectInScreen);
				} else if (parent instanceof JApplet
						|| parent instanceof JInternalFrame) {
					if (popupFrameRect == null) {
