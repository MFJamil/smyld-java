...
import com.smyld.gui.portal.engine.sources.PortalAppXMLSetReaderPESwing;
import com.smyld.gui.portal.engine.sources.PortalDBReaderPESwing;
import com.smyld.gui.portal.engine.sources.SettingsVariablesMapper;
...
public class PortalManager extends SMYLDObject implements Constants,
	...
	private void initSingleApp(String singleAppFile) throws Exception{
		...
		if ((mapper!=null)&&(mapper.getVariablesNumber()>0)){
			String xmlDocText = com.smyld.io.FileSystem.readStringFile(singleAppFile);
		...
		if (!appVersion.processVersion(args)) {
			...
			if (args[0].equals(RUN_ARG_SETTING_FILE)) {
				...
				if (settingsFile != null) {
					try {
						com.smyld.util.ProcessTimer timer = new ProcessTimer();
