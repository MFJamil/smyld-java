...
public class PEManagerService {
    ...
    public void processSingleApplication(String appFile,ApplicationArguments args){
        ...
        try {
            ...
            peGUIBuilder =  factory.getGUIBuilder(reader.getGUIToolkit(),reader.getType());
            logger.info("Application builder found for the given toolkit : " + peGUIBuilder.getAnnotation().guiToolkit() + " ==> " + peGUIBuilder.getAnnotatedClass().getName());
            ...
            Object generatedObject = handleApplicationFile(appFile);
            logger.info("Application reader was successfully created, invoking the Application generator now");
            // TODO below should be removed later and to use only PEApplication
            if (generatedObject instanceof PEApplication) {
                handleApplicationGeneration((PEApplication) generatedObject);
            }else if (generatedObject instanceof ApplicationReader){
                handleApplicationGeneration((ApplicationReader)generatedObject);
            }
        }catch(Exception ex){
            ex.printStackTrace();
    ...
    private ApplicationReader handleApplicationFile1(String appFile) throws Exception{
        // Handling the layout manager
        logger.info(" ================ " + peLayoutHandler.getAnnotatedClass());
    ...
    private Object handleApplicationFile(String appFile) throws Exception{
        // Handling the layout manager
        logger.info(" ================ " + peLayoutHandler.getAnnotatedClass());
        peLayoutHandler.instantiate();
        Object hostObj = peLayoutHandler.getHostObject();
    ...
    private void handleApplicationGeneration(PEApplication peApplication) throws Exception{
        // Handling the GUI Builder
        ...
        for (Method curMethod:hostObj.getClass().getMethods())
            if (curMethod.isAnnotationPresent(PEGenerateApplication.class)){
                for (Parameter curParam : curMethod.getParameters())
                    if (curParam.isAnnotationPresent(PEPreCompiledApplication.class)){
                        Object result = curMethod.invoke(hostObj,peApplication);
